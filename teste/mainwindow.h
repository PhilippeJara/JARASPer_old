...
#define MEMSIZE 100
#define MAXMICROL 256
#define MAXMICROC 41
#define BITSVAL 16
#define LOG2BIT log(BITSVAL)/log(2)
#define EXPONENT 4* log(BITSVAL)/log(16)
...
public:
  ...
  void assignToMemory( unsigned short); //só MDR
};
...
public:
  ...
  void assignFromMemory( std::string[], unsigned short); //só M[MAR]/ /MAR
};
...
public:
    std::string mem[MEMSIZE],REPLmem[MEMSIZE], opcode[MAXMICROL][MAXMICROC], Dopcode[100][MAXMICROC];
...
//void ComponentList::bindToWindow(MainWindow);
...
void loadOpcode(std::string [MAXMICROL][MAXMICROC],std::string);
...
int parseAllOpcodes(std::string[MAXMICROL][MAXMICROC], std::string[100][MAXMICROC]);
